...
export default function TalkToDataDiagram() {
  ...
  /* Arrow points UPWARD — matching original SVG: data flows up from sources → hub → conversational AI */
  const UpArrow = () => (
  ...
  const clientConfig = isCfo ? {
    ...
    gapText: 'Each system answers only for itself. No layer queries across the Jack Henry core, the Thought Machine ledger, Lending origination, and Snowflake at once to reconcile a single figure.',
    withText: 'One plain-language question returns one governed number, reconciled across every source — in seconds.',
  } : isNFCU ? {
    ...
    gapText: 'Queue health is in Dynamics 365. Staffing history is in HR. Volume trends are in Snowflake. One answer requires pulling three separate reports.',
    withText: 'One plain-language question returns a sourced answer across Dynamics 365, Genesys, Snowflake, and HR — in seconds.',
  } : {
    ...
    gapText: 'Answers require pulling reports from multiple dashboards or waiting on an analyst.',
    withText: 'One question, multiple live sources, one sourced answer in seconds.',
  };
    ...
    >
      ...
      <div className="flex-shrink-0" style={{ marginBottom: '0.6vh' }}>
        ...
        <p style={{ fontSize: fs.subtitle, color: '#666666', fontWeight: 500, margin: '0 0 0.3vh 0' }}>
          Powered by ConverseAI + Knowledge Hub
        </p>
        <p style={{ fontSize: fs.body, color: '#333333', fontWeight: 400, margin: 0, lineHeight: 1.5 }}>
          Ask a plain-language question and receive a sourced answer drawn simultaneously from multiple live data systems. No SQL. No analyst. No waiting.
        </p>
      </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        ...
        {/* Row 1: ConverseAI + Responsible AI */}
        <div className="flex" style={{ flex: '3 1 0', minHeight: 0, gap: 'min(1vw, 10px)' }}>
          ...
          >
            <div className="flex items-baseline flex-shrink-0" style={{ marginBottom: 'min(0.8vh, 8px)', gap: 'min(1vw, 12px)' }}>
              <span style={{ fontSize: fs.label, color: 'var(--color-brand)', fontWeight: 700, letterSpacing: '1.5px', whiteSpace: 'nowrap' }}>CONVERSEAI</span>
              <span style={{ fontSize: fs.labelSm, color: '#666666', fontWeight: 400 }}>Enterprise conversational platform with role-aware interactions</span>
            </div>
            <div className="flex flex-1 min-h-0" style={{ gap: 'min(0.6vw, 8px)' }}>
              {[
                { name: 'Intent Recognition', desc: 'NLU parsing, entity extraction from questions' },
                { name: 'Query Decomposition', desc: 'Splits complex questions into parallel multi-source queries' },
                { name: 'Answer Synthesis', desc: 'Grounded, cited responses with source attribution' },
              ].map((item) => (
                <div
                  key={item.name}
          ...
          >
            ...
            <div className="flex-1 flex flex-col justify-around">
              {['PII Masking at Query Time', 'Role-Based Access Control', 'Per-Claim Source Attribution', 'Response Evaluation'].map((item) => (
        ...
        >
          <div className="flex items-baseline flex-shrink-0" style={{ marginBottom: 'min(0.6vh, 6px)', gap: 'min(1vw, 12px)' }}>
            <span style={{ fontSize: fs.label, color: 'var(--color-brand)', fontWeight: 700, letterSpacing: '1.5px', whiteSpace: 'nowrap' }}>KNOWLEDGE HUB</span>
            <span style={{ fontSize: fs.labelSm, color: '#666666', fontWeight: 400 }}>Multi-modal RAG platform with PII masking, source grounding, and response evaluation</span>
          </div>
          <div className="flex flex-1 min-h-0" style={{ gap: 'min(0.5vw, 7px)' }}>
            {[
              { name: 'Pre-built Connectors', desc: clientConfig.connectorDesc },
              { name: 'PII Disguise', desc: clientConfig.piiDesc },
              { name: 'Prompt Orchestrator', desc: 'Multi-source query routing' },
              { name: 'Response Evaluation', desc: 'Grounding and accuracy' },
              { name: 'Knowledge Graph', desc: 'Entity relationships' },
            ].map((item) => (
              <div
                key={item.name}
                className="flex-1 flex flex-col justify-center"
              ...
              >
                <p style={{ fontSize: fs.cardTitle, color: 'var(--color-brand)', fontWeight: 700, margin: '0 0 min(0.3vh, 3px) 0' }}>{item.name}</p>
                <p style={{ fontSize: fs.cardDesc, color: '#666666', margin: 0, lineHeight: 1.4 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex" style={{ gap: 'min(1vw, 10px)' }}>
          ...
          >
            ...
            <p style={{ fontSize: fs.small, color: '#333333', margin: 0, lineHeight: 1.5 }}>
              <span style={{ fontWeight: 700 }}>With Converged Conversation: </span>
