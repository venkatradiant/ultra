...
import { makeChatResolver } from './presentationChat';
import { DeckContext, makeHelpers } from './deckContext';
...
/**
 * Full-screen, board-ready slide deck (spec §15a). Launched from the hero
 * briefing card via "View Full Briefing"; the header X is the only exit and
 * returns to Conversation Mode. Portaled to <body> so it truly covers the app
 * shell.
 *
 * Shared by both ESFCU decks. Everything that differs between them arrives as
 * props: the seven slide components, the deck copy, the narration script, the
 * scripted Q&A bank, the lineage figures, the event namespace, and which TTS
 * voice bank to read from. What is shared is everything a deck IS — the fixed
 * 1280x720 stage and its scaling, keyboard and dot navigation, the play loop
 * and its hand-off to narration, the transcript, the chat sheet, the lineage
 * overlay, the closing modal and the long-form PDF report.
 *
 * The slides themselves are NOT shared. They are where the story lives, and the
 * two stories have nothing in common but their shape.
 */
export default function PresentationMode({
  ...
  const chat = useMemo(() => makeChatResolver(chatBank, slideIds), [chatBank, slideIds]);
  // Slide interactions (ask-about, open-step, open-lineage) reach the deck
  // through this rather than through global window events, so two ESFCU decks
  // can never hear each other's.
  const deckHelpers = useMemo(() => makeHelpers(eventNamespace), [eventNamespace]);
  const { resolve, findById, suggestionsFor, greeting, slideIdForIndex } = chat;
  // Speaker notes for the current slide (mirrors slideIdForIndex).
  const narrationForIndex = useCallback(
    (i) => narrationData.narration[slideIdForIndex(i)] || '',
    [narrationData, slideIdForIndex],
  );
  ...
  const narr = useNarration({ persona: ttsPersona, version: narrationVersion });
  ...
  }, [narr, N, narrationForIndex, slideIdForIndex]);
  ...
  useEffect(() => {
    function fit() {
      ...
      setScale(f > 0 ? f : 1);
      // The report stacks slides vertically, so it is width-constrained only.
      // Capped at 1 so the leave-behind never renders larger than designed.
      const r = Math.min(1, (el.clientWidth - 48) / STAGE_W);
      setReportScale(r > 0 ? r : 1);
    }
    fit();
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      // In the long-form report there are no slides to step through, and Escape
      // should return to the deck rather than dump the CEO out of the briefing.
      if (reportOpen) {
        if (e.key === 'Escape') { e.preventDefault(); setReportOpen(false); }
        return;
      }
  ...
  }, [next, prev, onClose, closingOpen, reportOpen]);
  ...
  useEffect(() => {
    ...
    window.addEventListener(`${eventNamespace}:open-lineage`, open);
    return () => window.removeEventListener(`${eventNamespace}:open-lineage`, open);
  }, [narr, eventNamespace]);
  ...
  useEffect(() => {
    ...
    window.addEventListener(`${eventNamespace}:open-chat`, h);
    return () => window.removeEventListener(`${eventNamespace}:open-chat`, h);
  }, [eventNamespace]);
  ...
  useEffect(() => {
    ...
    window.addEventListener(`${eventNamespace}:open-closing`, h);
    return () => window.removeEventListener(`${eventNamespace}:open-closing`, h);
  }, [narr, eventNamespace]);
  ...
  // Download PDF → the long-form leave-behind (spec §15a, "Export to PDF").
  //
  // The USSFCU deck downloads a pre-made static PDF from /public. There is no
  // such artifact for ESFCU and shipping a stub would 404, so this generates the
  // report instead: the same seven slides' content, the same approved design,
  // re-laid-out as one continuous scrolling page via the deck's own `printmode`
  // scope, then handed to the browser's print dialog (Save as PDF). Same data,
  // no new dependency, nothing to keep in sync.
  const openReport = () => {
    narr.cancel();
    setPlaying(false);
    setChatOpen(false);
  ...
  const printReport = () => {
    // Let the report layout paint before the print dialog snapshots it.
    window.requestAnimationFrame(() => window.setTimeout(() => window.print(), 60));
  };
  ...
  const deck = (
    <div
      className={`esfcu-pm-root${reportOpen ? ' printmode' : ''}`}
      role="dialog"
      aria-modal="true"
      aria-label={pres.meta.deckLabel || pres.meta.eyebrow}
    >
      {/* Header */}
      <div className="topbar">
        <div className="brand">
          <span className="hlogo"><img src="/logos/esfcu-logo.svg" alt="Educational Systems Federal Credit Union" /></span>
          <span className="hsub">{pres.meta.eyebrow}</span>
        </div>
        <div className="topright">
          <span className="conf-line">{pres.meta.confidenceLine}</span>
          <button type="button" className="pdfbtn" onClick={openReport} title="Open the long-form report to save as PDF">
      ...
      {/* Long-form report bar — only in report mode, hidden when printing. */}
      {reportOpen ? (
        <div className="pm-reportbar">
          <span className="pm-reportbar-title">
            {pres.meta.reportTitle || 'Long-form report'}
            <span className="pm-reportbar-sub">{pres.meta.confidenceLine}</span>
          </span>
          <span className="pm-reportbar-actions">
            <button type="button" className="pm-reportbar-print" onClick={printReport}>Save as PDF</button>
            <button type="button" className="pm-reportbar-back" onClick={() => setReportOpen(false)}>Back to the deck</button>
          </span>
        </div>
      ) : null}

      {/* Stage — one slide at a time in the deck, every slide stacked in the report. */}
      <div className="stagewrap" ref={stagewrapRef}>
        {reportOpen ? (
          <div
            className="pm-reportflow"
            style={{ width: STAGE_W * reportScale, height: STAGE_H * SLIDES.length * reportScale }}
          >
            <div className="stage" style={{ transform: `scale(${reportScale})`, transformOrigin: 'top left' }}>
              {SLIDES.map((Slide, i) => (
                <Slide key={i} active />
              ))}
            </div>
      ...
      <DeckLineageOverlay open={lineageOpen} onClose={() => setLineageOpen(false)} figures={lineageFigures} />
  ...
  return createPortal(
    <DeckContext.Provider value={deckHelpers}>{deck}</DeckContext.Provider>,
    document.body,
  );
}
