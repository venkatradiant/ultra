...
//
// `makeChatResolver(bank, slideIds)` binds a deck's own Q&A bank and slide
// order. Each ESFCU deck has its own; the matching logic does not differ.
...
export function makeChatResolver(bank, slideIds) {
  const slideIdForIndex = (idx) => slideIds[idx] || slideIds[0];
  ...
  const greeting = () => bank.global.greeting;

  const suggestionsFor = (slideId) => bank.slides[slideId]?.suggested || bank.global.default.followups;
  ...
  return { slideIdForIndex, greeting, suggestionsFor, findById, resolve };
}
