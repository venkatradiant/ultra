...
// Forked from CapmKpiCarousel for ESFCU, then shared across both ESFCU personas.
// The one addition is the honest-data marker: every tile says whether its figure
// is ESFCU's, the industry's, or illustrative, because spec §7 requires each KPI
// to name its source and the data posture requires the three never to be
// confused.
//
// The marker comes from a `meta` map keyed by stat id, not from the manifest's
// `ui.stats`, so the shared StatTile type stays untouched. Each persona binds
// its own kpis.json (see ceo/KpiCarousel.jsx, cro/KpiCarousel.jsx) — the
// manifest slot takes a component, not props, so the binding has to happen in a
// wrapper rather than at the call site.

const PER_PAGE = 4;

function StatCard({ stat, onClick, meta }) {
  const Icon = stat.icon;
  const isClickable = !!stat.chipText;
  const prov = meta ? provenanceOf(meta.provenance) : null;

  return (
    <div
      onClick={isClickable ? () => onClick(stat.chipText) : undefined}
      title={meta ? `${meta.sourceLabel} · ${meta.calc} · ${meta.sourceCitation}` : undefined}
      ...
      {meta ? (
        <div className="mt-1.5 space-y-1 border-t border-border-subtle pt-1.5">
          <div className="flex items-center gap-1">
            <span
              className={`rounded px-1 py-px text-[8px] font-bold uppercase tracking-wide ${prov.className}`}
              title={prov.title}
            >
              {prov.label}
            </span>
...
export default function KpiCarousel({ visible, onStatClick, stats, meta = {} }) {
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-2 lg:flex">
          {visibleStats.map((stat) => (
            <StatCard key={stat.id} stat={stat} onClick={onStatClick} meta={meta[stat.id]} />
          ))}
        </div>
      </motion.div>
    </AnimatePresence>
