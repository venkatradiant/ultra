...
// `esfcu-ceo-deck:open-lineage` pattern so slides never import deck internals.
export function askAbout(seedId) {
  window.dispatchEvent(new CustomEvent('esfcu-ceo-deck:open-chat', { detail: { seed: seedId } }));
}
...
export function openStep(step) {
  window.dispatchEvent(new CustomEvent('esfcu-ceo-deck:open-closing', { detail: { step } }));
}
