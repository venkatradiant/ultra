...
// Keep in step with SLIDE_IDS in presentationChat.js.
const SLIDES = [
  SlideSituational,
  SlideEvidence,
  SlideTrajectory,
...
export default function PresentationMode({ onClose }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('esfcu-ceo-deck:open-lineage', open);
    return () => window.removeEventListener('esfcu-ceo-deck:open-lineage', open);
  }, [narr]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('esfcu-ceo-deck:open-chat', h);
    return () => window.removeEventListener('esfcu-ceo-deck:open-chat', h);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener('esfcu-ceo-deck:open-closing', h);
    return () => window.removeEventListener('esfcu-ceo-deck:open-closing', h);
  }, [narr]);
  ...
  const deck = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="ESFCU CEO Board Briefing"
    >
      {/* Header */}
      ...
      {reportOpen ? (
        <div className="pm-reportbar">
          <span className="pm-reportbar-title">
            Board briefing — long-form report
