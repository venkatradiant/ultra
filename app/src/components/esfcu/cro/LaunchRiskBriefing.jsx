 ...
 */
export default function LaunchRiskBriefing() {
  const fired = useRef(false);

  useEffect(() => {
    if (fired.current) return;
    fired.current = true;
    window.dispatchEvent(new CustomEvent('esfcu-cro:open-presentation'));
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[12px] font-semibold text-text">Risk briefing opened</p>
        <p className="text-[11px] leading-snug text-text-muted">
          Seven slides for the supervisory committee, the trust ribbon with the coverage gap shown
          honestly, and the recommended path. Close it with the header × to come back here.
        </p>
      </div>
      <button
        type="button"
        onClick={() => window.dispatchEvent(new CustomEvent('esfcu-cro:open-presentation'))}
