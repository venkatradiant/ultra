  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 bg-surface-2 rounded-full h-1.5 overflow-hidden">
        <div className={`h-full rounded-full ${v >= 95 ? 'bg-red-400' : v >= 85 ? 'bg-amber-400' : 'bg-emerald-400'}`} style={{ width: pct }} />
      </div>
...
export default function StaffingScenarioTable() {
  const scenarios = metrics.scenarios;
  return (
    <div className="bg-surface rounded-xl border border-border overflow-hidden">
      <div className="px-4 py-3 border-b border-border-subtle">
        <p className="text-xs font-semibold text-text-muted">Staffing Scenarios vs Service Level Impact — Dec 8–15</p>
        <p className="text-[10px] text-text-subtle mt-0.5">Source: Workforce Management forecasting · Budget System · BPO capacity feed</p>
      </div>
      ...
      <div className="px-4 py-2.5 border-t border-border-subtle bg-brand/[0.04]">
        <p className="text-[10px] text-brand font-semibold">→ The hybrid (9 OT + 12 BPO + deflection) holds 79% at the lowest cost and preserves budget headroom for unplanned batches.</p>
      </div>
    </div>
  );
}
