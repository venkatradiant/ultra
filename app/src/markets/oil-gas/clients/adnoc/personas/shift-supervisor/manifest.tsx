/**
 * Persona: Shift Supervisor, RFCC-2 and DCU-3 — ADNOC (illustrative).
 *
...
import signals from '@/data/adnoc/shift-supervisor/signals.json';
import dataSources from '@/data/adnoc/_shared/dataSources.json';
import capabilityCallouts from '@/data/adnoc/shift-supervisor/capabilityCallouts.json';
import kpis from '@/data/adnoc/shift-supervisor/kpis.json';
...
const manifest: PersonaManifest = {
  ...
  ui: {
    ...
    goldenPathChip: {
      adnoc_sup_greeting: 'Show me the expired hot-work permits',
      adnoc_sup_expired_permits: 'Draft the stop-work notice',
      adnoc_sup_stop_work: 'What is the status on the DCU-3 confined space?',
      adnoc_sup_confined_space: 'Remind the crew of the gas test',
      adnoc_sup_gas_reminder: 'What has been assigned to me?',
      adnoc_sup_assigned: 'How is my muster point looking?',
      adnoc_sup_on_location: 'Draft the stop-work notice',
      adnoc_sup_extension: 'What has been assigned to me?',
    },
    flowKeyToCapabilityTrigger: {
      adnoc_sup_greeting: 'home_load',
      adnoc_sup_expired_permits: 'ask_turn_1',
      adnoc_sup_stop_work: 'ask_turn_4',
      adnoc_sup_extension: 'ask_turn_4',
      adnoc_sup_on_location: 'ask_turn_2',
      adnoc_sup_confined_space: 'ask_turn_2',
      adnoc_sup_gas_reminder: 'ask_turn_4',
      adnoc_sup_assigned: 'ask_turn_4',
      adnoc_sup_muster: 'ask_turn_5',
    },
    stats,
    signalToChip: {
      'SIG-ARAMCO-SUP-001': 'Show me the expired hot-work permits',
      'SIG-ARAMCO-SUP-002': 'What is the status on the DCU-3 confined space?',
  ...
  inlineComponents: (msg) => {
    ...
    // Fassy drills in. Same rule across all four ADNOC personas.
    if (k === 'adnoc_sup_expired_permits' || k === 'adnoc_sup_on_location') {
      out.push(<FlaggedJobsTable key="flagged" />);
    }
    if (k === 'adnoc_sup_confined_space' || k === 'adnoc_sup_gas_reminder') {
      out.push(<LazyIndoorViewer key="indoor" />);
      out.push(<PermitDetailCard key="permit" compact={k === 'adnoc_sup_gas_reminder'} />);
    }
    if (k === 'adnoc_sup_assigned' || k === 'adnoc_sup_stop_work' || k === 'adnoc_sup_extension') {
      out.push(<PrioritizedActionCards key="actions" />);
    }
    if (k === 'adnoc_sup_muster') out.push(<MusterBoard key="muster" />);
